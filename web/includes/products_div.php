...
// 1️⃣ If search term exists
if (!empty($_GET['search'])) {
    $search = trim($_GET['search']);
    $conditions[] = "lpa_stock_name LIKE ?";
    $params[] = "%$search%";
    $types .= "s";
}

// 2️⃣ If category filter exists
if (!empty($_SESSION['category'])) {
    $conditions[] = "lpa_stock_cat = ?";
...
<div class="products">
    <?php if ($result && $result->num_rows > 0): ?>
        <?php while ($row = $result->fetch_assoc()): ?>
            <div class="product">
                <img src="<?php echo htmlspecialchars($row['lpa_stock_image']); ?>" 
                     alt="<?php echo htmlspecialchars($row['lpa_stock_name']); ?>" 
                     class="image">
